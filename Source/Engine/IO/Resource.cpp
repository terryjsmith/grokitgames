...
bool Resource::Delete(std::string filename) {
...
void Resource::Write(void *data, unsigned long bytes) {
    if (m_fp == 0) {
        return;
    }
...
void Resource::Read(void* dest, unsigned long bytes) {
    if (m_fp == 0) {
        return;
    }
