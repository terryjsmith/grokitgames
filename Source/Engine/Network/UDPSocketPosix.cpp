...
bool UDPSocketPosix::Connect(std::string server, int port) {
    ...
    if(m_socket < 0) {
        errorSystem->HandleError(new Error(Error::MSG_WARN, "Unable to create socket for server", server));
        return(false);
    }
    ...
    if(host == 0) {
        errorSystem->HandleError(new Error(Error::MSG_WARN, "Unable to get host", server));
        return(false);
    }
    ...
    if(fcntl(m_socket, F_SETFL, flags) != 0) {
        errorSystem->HandleError(new Error(Error::MSG_WARN, "Unable to set non-blocking connection type.", server));
        return(false);
    }
    ...
    if(::connect(m_socket, (struct sockaddr*)&m_sockaddr, sizeof(m_sockaddr)) < 0) {
        errorSystem->HandleError(new Error(Error::MSG_WARN, "Unable to connect to host", server));
        return(false);
    }
...
void UDPSocketPosix::Listen(int port) {
    ...
    if(m_socket < 0) {
        errorSystem->HandleError(new Error(Error::MSG_WARN, "Unable to initialize socket."));
        return;
    }
    ...
    if (setsockopt(m_socket, SOL_SOCKET, SO_REUSEADDR, &enable, sizeof(int)) < 0) {
        ...
        errorSystem->HandleError(new Error(Error::MSG_WARN, "Unable to re-use socket address."));
        return;
    }
    ...
    if(bind(m_socket, (struct sockaddr *)&servaddr, sizeof(servaddr)) < 0) {
        ...
        errorSystem->HandleError(new Error(Error::MSG_WARN, "Unable to bind socket."));
        return;
    }
}
