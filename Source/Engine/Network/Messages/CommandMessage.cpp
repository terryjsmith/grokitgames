...
CommandMessage::CommandMessage() {
    ...
    m_envelope.type = NetworkMessages::Message_CommandMessage;
}
...
void CommandMessage::OnSend() {
    ...
    uint32_t type = m_command->GetCommandType();
...
void CommandMessage::OnReceive() {
    ...
    if (state == Command::COMMAND_START) {
        start = tick - floor(GIGA_TICKS_PER_SECOND * session->info.pingTime) - NETWORK_SNAPSHOT_RENDER_LAG;
        printf("Current tick: %d, setting start to %d\n", tick, start);
    }
    else {
        end = tick - floor(GIGA_TICKS_PER_SECOND * session->info.pingTime) - NETWORK_SNAPSHOT_RENDER_LAG;
