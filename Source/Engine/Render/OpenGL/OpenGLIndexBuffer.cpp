...
void OpenGLIndexBuffer::Create(int count, void *data) {
    ...
    GL_CHECK(glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(unsigned int) * m_indexCount, data, GL_STATIC_DRAW));
}

void OpenGLIndexBuffer::SetData(int count, void *data) {
    m_indexCount = count;
...
uint32_t* OpenGLIndexBuffer::GetData(int& count) {
    uint32_t* indices = (uint32_t*)malloc(sizeof(unsigned int) * m_indexCount);
    GL_CHECK(glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, m_buffer));
    
    int size = 0;
    GL_CHECK(glGetBufferParameteriv(GL_ELEMENT_ARRAY_BUFFER, GL_BUFFER_SIZE, &size));
    GL_CHECK(glGetBufferSubData(GL_ELEMENT_ARRAY_BUFFER, 0, sizeof(unsigned int) * m_indexCount, indices));
