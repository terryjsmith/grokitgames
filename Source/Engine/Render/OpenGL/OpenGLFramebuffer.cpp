...
void OpenGLFramebuffer::SetTexture(Texture2D* texture, int type, int slot) {
    // If there is an existing texture in this slot, destroy and re-assign
    if((slot - GL_COLOR_ATTACHMENT0) < m_textures.size()) {
        m_textures[slot - GL_COLOR_ATTACHMENT0] = texture;
    }
    
    // Attach the texture to the FBO
    GL_CHECK(glBindFramebuffer(GL_FRAMEBUFFER, m_framebuffer));
    GL_CHECK(glFramebufferTexture2D(GL_FRAMEBUFFER, slot, GL_TEXTURE_2D, texture->GetTexture(), 0));
